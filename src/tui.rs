...
pub fn run_tui() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        ...
        terminal.draw(|f| {
            ...
            // If there are no tags, give more space to the preview by increasing
            // the right-hand column percentage.
            let left_pct: u16 = if unique_tags.is_empty() { 30 } else { 40 };
            let chunks = Layout::default().direction(Direction::Horizontal).constraints([Constraint::Percentage(left_pct), Constraint::Percentage(100 - left_pct)].as_ref()).split(content_area);
